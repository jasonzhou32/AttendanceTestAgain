                ...
                <div class="card-body">
                  <form method="post">
                    <div class="form-group row mb-3">
                      <div class="col-xl-6">
                        <label class="form-control-label">Search Volunteer<span class="text-danger ml-2">*</span></label>
                        <input type="text" name="search" class="form-control mb-3" placeholder="Search for a volunteer...">
                      </div>
                      ...
                      <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                        ...
                        <tbody>
                          ...
                          if(isset($_POST['view'])) {
                            ...
                            $query = "SELECT va.*, v.volunteer_name, c.class_name 
                                      FROM tblvolunteerattendance va 
                                      JOIN volunteers v ON va.volunteer_id = v.volunteer_id 
                                      JOIN classes c ON va.class_id = c.class_id 
                                      WHERE (LOWER(v.volunteer_name) LIKE LOWER('%$searchTerm%'))";
                        
                            if ($type == "2") { // Single Date Attendance
                                $singleDate = $_POST['singleDate'];
                                $query .= " AND DATE(va.dateTimeTaken) = '$singleDate'";
                            } elseif ($type == "3") { // Date Range Attendance
                                $fromDate = $_POST['fromDate'];
                                $toDate = $_POST['toDate'];
                                $query .= " AND DATE(va.dateTimeTaken) BETWEEN '$fromDate' AND '$toDate'";
                            }
